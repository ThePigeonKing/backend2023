...
from .api import get_city_choices, get_weather_data
from .forms import CitySubscriptionForm, CommentForm, RegisterForm
...
def main_page(request):
    ...
    city_names = [
        city["name"] for city in cities.values() if city["population"] > 1000000
    ]
...
def manage_subscriptions(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if CitySubscription.objects.filter(
    ...
    return render(
        request,
        "weather_app/manage_subscriptions.html",
        {
            "form": form,
...
def unsubscribe(request, subscription_id):
    subscription = get_object_or_404(
        CitySubscription, id=subscription_id, user=request.user
    )
    if request.method == "POST":
        subscription.delete()
...
def city_detail(request, city_name):
    ...
    subscription = get_object_or_404(
        CitySubscription, user=request.user, city_name=city_name
    )
    weather_report = get_weather_data(city_name)
    sunrise_time = datetime.fromtimestamp(weather_report["sys"]["sunrise"], tz=timezone.utc)
    sunset_time = datetime.fromtimestamp(weather_report["sys"]["sunset"], tz=timezone.utc)

    # Add the converted times to the report dictionary
    weather_report["formatted_sunrise"] = sunrise_time.strftime("%H:%M")
    weather_report["formatted_sunset"] = sunset_time.strftime("%H:%M")
    ...
    comments = Comment.objects.filter(city_subscription=subscription).order_by(
        "-created_at"
    )
    return render(
        request,
        "weather_app/city_detail.html",
        {
...
def edit_comment(request, comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('city_detail', city_name=comment.city_subscription.city_name)
    else:
        form = CommentForm(instance=comment)

    return render(request, 'weather_app/edit_comment.html', {
        'form': form,
        'comment': comment,
    })